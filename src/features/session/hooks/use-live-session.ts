...
function applyRuntimeDiagnostics(payload: DesktopRuntimeDiagnosticsPayload) {
  useSessionViewStore.getState().applyRuntimeDiagnostics({
    ...
    microphoneInputActive: payload.microphone_input_active,
    systemInputActive: payload.system_input_active,
    lastMicrophoneInputAt: payload.last_microphone_input_at,
    lastSystemInputAt: payload.last_system_input_at,
